...
class AC_Latest_Portfolio extends WP_Widget
{
  function AC_Latest_Portfolio()
  {
    ...
    parent::__construct('AC_Latest_Portfolio', 'AC Latest Portfolio Item', $widget_ops);
  }
      ...
      echo $before_title . $title . $after_title;;
		...
		if (! empty($posts)) {
			...
			foreach ( $posts as $post ) : setup_postdata( $post ); 
				...
				$post_type = 'ac_portfolio';
				$show_read_more = true;
				$image_aspect_ratio = AC_IMAGE_RATIO_SQUARE;				
				$template_params = compact('show_title', 'show_excerpt', 'column_count', 'post_category', 'cols_class', 'cols', 'excerpt_length', 'post_type', 'show_read_more', 'image_aspect_ratio');
