...
<header class="banner navbar navbar-default topnavbar <?php echo esc_attr(shoestrap_navbar_class()); ?> <?php echo esc_attr(shoestrap_outer_container_class('header')); ?>" role="banner">
  ...
  <div class="<?php echo esc_attr(shoestrap_container_class('header')); ?> navbar-outer">
    ...
    <nav class="nav-main navbar-collapse collapse" role="navigation">
			...
			<?php	    
        // Get the dekstop nav type
        $navbar_desktop_type = shoestrap_getVariable( 'navbar_desktop_type' );
	    ?>
	    
	    <?php
		    // Desktop reveal nav button, goes before search button
				if ($navbar_desktop_type == 'overlay') {
					// Show the hamburger
					?>
			...
			<?php // Search ?>
    	<?php if ( ($navbar_desktop_type == 'navbar') && (shoestrap_getVariable( 'navbar_search' ) == 1 ) ) : ?>
